...
const validation = (req, res, next) => {
  try {
    ...
    if (validator.isEmpty(errors.email_error)) {
      ...
      RegisterSchema.registeration
        .findOne({ email_id })
        .then((exist) => {
          if (exist) {
            //check account is activated or not
            RegisterSchema.registeration
              .findOne({ email_id, status: 1 })
              .then((exist) => {
                if (exist) {
                  next();
                } else {
              ...
              .catch((err) => {
                console.log(
                  `got error when checking accout is activated  in forgotpassword validation : ${err}`
                );
              });
          } else {
        ...
        .catch((err) => {
          console.log(
            `got error when performing database operation in forgotpassword validation : ${err}`
          );
        });
    } else {
      res.json({ errors });
...
const sendEmail = (req, res, next) => {
  ...
  try {
    ...
    let mailOptions = {
      ...
      html: `<h4>Forgot  your password ?</h4>
        <div>
...
router.post("/forgot", validation, sendEmail, (req, res) => {
  const { email_id } = req.body;
  res.json({
    success: `A link to reset your password has been sent to  ${email_id} click that link within 1 hour`,
  });
});
...
const pass_validation = (req, res, next) => {
  ...
  errors.cpass_error = validations.validate_confirm_password(
    cpassword,
    password
  );

  validator.isEmpty(errors.pass_error) && validator.isEmpty(errors.cpass_error)
    ? next()
    : res.json({ errors });
};
